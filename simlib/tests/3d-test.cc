...
//const Speed    v0(0, 4200, 1700);   // poèáteèní rychlost dru¾ice
...
MyWorld::MyWorld() {  // vytvoøíme digitální svìt
  ...
  for(unsigned int i=0; i<n; i++)         // zapneme silové pùsobení
...
int main()
{
  ...
  SetMethod("abm4");
  SetStep(1,50);                  // krok
  SetAccuracy(1e-5);              // pro test není nutná vysoká pøesnost
  Run();                          // simulace
  return 0;
}
